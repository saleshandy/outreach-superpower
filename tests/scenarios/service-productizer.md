...
1. **Agency happy path.** User: *"I run an SEO agency. Productize my link-building service."* Workspace empty.
   - Expected: skill asks for website -> invokes strategy-architect first -> returns -> lists extracted services -> user picks "link-building" -> productizes -> asks if user wants the 3-email sequence -> on yes, auto-chains to email-sequence-generator.
...
3. **Service not in extracted list.** User picks "Custom: AI strategy consulting."
   - Expected: skill accepts, asks 2-3 follow-up questions to fill gaps (target outcome, scope, timeline).
